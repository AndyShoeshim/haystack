...
def create_query_pipeline(template: str, document_store: InMemoryDocumentStore) -> Pipeline:
    ...
    routes = [
        {
            "condition": "{{ 'bali' in query.lower() }}",
            "output": "{{True}}",
            "output_name": "ok",
            "output_type": str,
        },
        {
            "condition": "{{ 'bali' not in query.lower() }}",
            "output": "{{True}}",
            "output_name": "ko",
            "output_type": str,
        },
    ...
    rag.connect("router.ok", "prompt.ok")
    rag.connect("router.ko", "prompt.ko")

    # rag.connect("router.does_not_contain_bali", "prompt")
...
def main():
    ...
    {% if ko %}
        Answer the question telling that keyword bali is needed to answer the question. Do it in italian.
        Question: {{ query }}
    {% endif %}
    ...
    while True:
        ...
        result = query_pipeline.run(
            ...
            include_outputs_from=["router", "prompt", "generator"],
        )
